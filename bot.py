...
def load_state():
    try:
        ...
        base = _default_state()
        base.update(s)
        base["settings"] = {**_default_state()["settings"], **s.get("settings", {})}
        return base
    except Exception:
...
def _token_price_line(tokens):
    """Best-effort live price for the top token, so you can act fast. Silent on fail."""
    if not tokens:
        return None
    tk = tokens[0]["ticker"]
    try:
...
def format_event_alert(e, a):
    """Format one radar event (post/announcement) + its read into an alert."""
    imp = a["impact"]
...
HELP = (
    ...
    "/alerts on|off — auto signal &amp; news alerts\n"
    "/settings — show/change account, risk%, rr, timeframe\n\n"
    f"{config.DISCLAIMER}"
)
...
def cmd_analyze(chat_id, args):
    ...
    coin = args[0]
    tf = args[1] if len(args) > 1 else STATE["settings"]["timeframe"]
    s = STATE["settings"]
    try:
...
def cmd_risk(chat_id, args):
    ...
    try:
        if len(nums) == 2:                       # entry, stop -> use defaults
            entry, sl = nums
...
def cmd_watch(chat_id, args):
    ...
    with _STATE_LOCK:
        for c in args:
            c = c.upper()
            if c not in STATE["watchlist"]:
                STATE["watchlist"].append(c)
        save_state(STATE)
        wl = ", ".join(STATE["watchlist"])
    send_message(chat_id, f"✅ Watching: {esc(wl)}")
...
def cmd_unwatch(chat_id, args):
    ...
    with _STATE_LOCK:
        for c in args:
            c = c.upper()
            if c in STATE["watchlist"]:
                STATE["watchlist"].remove(c)
        wl = ", ".join(STATE["watchlist"]) or "(empty)"
        save_state(STATE)
...
def cmd_news(chat_id, args):
    try:
        ...
        rel = news_mod.filter_relevant(heads, STATE["watchlist"]) or heads
...
def cmd_alerts(chat_id, args):
    ...
    with _STATE_LOCK:
        STATE["alerts_on"] = on
        if on and chat_id not in STATE["subscribers"]:
            STATE["subscribers"].append(chat_id)
        if not on and chat_id in STATE["subscribers"]:
            STATE["subscribers"].remove(chat_id)
        save_state(STATE)
    send_message(chat_id, f"🔔 Alerts turned {'ON' if on else 'OFF'}.")
    ...
    try:
        with _STATE_LOCK:
            if field == "timeframe":
                ...
                STATE["settings"][field] = parse_num(val)
            save_state(STATE)
        send_message(chat_id, f"✅ {esc(field)} set to {esc(STATE['settings'][field])}")
    except ValueError:
        send_message(chat_id, "That value doesn't look like a number.")
...
def cmd_radar(chat_id, args):
    # /radar on | off  toggles the auto radar; /radar with no args shows latest.
    if args and args[0].lower() in ("on", "off"):
        ...
        with _STATE_LOCK:
            ...
            if on and chat_id not in STATE["subscribers"]:
                STATE["subscribers"].append(chat_id)
            save_state(STATE)
        return send_message(chat_id, f"📡 Radar auto-alerts turned {'ON' if on else 'OFF'}.")
...
def cmd_sources(chat_id, args):
    with _STATE_LOCK:
        extra_x = list(STATE.get("tracked_x", []))
        radar_on = STATE.get("radar_on", True)
    st = radar.source_status(extra_x_handles=extra_x)
    lines = [f"<b>📡 Radar sources</b> (auto-alerts: {'ON' if radar_on else 'OFF'}, "
...
def scanner_loop():
    ...
    while True:
        try:
            now = time.time()
            alerts_on = STATE.get("alerts_on", True)

            # The radar is the fast, "be first" loop — check it often.
            if STATE.get("radar_on", True) and now - last_radar >= config.RADAR_SCAN_INTERVAL_SEC:
                last_radar = now
                scan_radar_and_alert()

            if alerts_on and now - last_market >= config.SCAN_INTERVAL_MIN * 60:
                last_market = now
                scan_market_and_alert()

            if alerts_on and now - last_news >= config.NEWS_SCAN_INTERVAL_MIN * 60:
                last_news = now
                scan_news_and_alert()
        except Exception:
            traceback.print_exc()
...
def scan_market_and_alert():
    with _STATE_LOCK:
        ...
        last_sig = dict(STATE.get("last_signal", {}))
    for coin in wl:
        try:
            a = strategy.analyze(coin, timeframe=s["timeframe"],
                                 account=s["account"], risk_pct=s["risk_pct"], rr=s["rr"])
        ...
        # Only alert when the signal is new or flipped direction / strengthened.
        if prev == a["score"]:
            continue
        last_sig[a["symbol"]] = a["score"]
        broadcast("🚨 <b>Signal alert</b>\n" + format_analysis(a))
    with _STATE_LOCK:
...
def scan_news_and_alert():
    ...
    try:
        ...
        return
    rel = news_mod.filter_relevant(heads, wl)
    fresh = [h for h in rel if h["id"] not in seen]
    for h in fresh[:5]:
        broadcast(format_news([h], header=f"📰 News · {h.get('matched','crypto')}"))
        seen.add(h["id"])
    with _STATE_LOCK:
...
def _should_alert(e, a):
    """Decide whether a radar event is worth pinging about."""
    src = e["source"]
    has_token = bool(a["tokens"])
    directional = a["impact"]["direction"] != "NEUTRAL"
    if src in ("binance_listing", "exchange_listing"):
        return True                                  # listings are high-signal
    if src in ("truth_social", "x"):                 # influencers: even 1% crypto
        return has_token or a["relevance"] >= config.RADAR_MIN_RELEVANCE
    if src == "news":                                # news: only if actionable
...
def scan_radar_and_alert():
    with _STATE_LOCK:
        if not STATE.get("radar_on", True):
            return
    ...
    # First run ever: learn what's already out there, don't alert on history.
    if not seeded:
        with _STATE_LOCK:
    ...
    fresh.sort(key=lambda e: e["ts"])                # chronological order
    sent = 0
    for e in fresh:
        a = nlp.analyze_text(e["text"])
        if _should_alert(e, a) and sent < 6:
            try:
                broadcast(format_event_alert(e, a))
                sent += 1
            except Exception:
                traceback.print_exc()
        seen.add(e["id"])
